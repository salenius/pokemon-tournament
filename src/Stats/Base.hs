...
newtype BaseStatValue = BaseStatValue {baseStatValueToInt :: Int} deriving (Eq,Show,Ord)

data BaseStatMap v = BaseStatMap
  {
    hp :: v
  , attack :: v
  , defence :: v
...
baseToModif :: BaseStat -> Maybe ModifStat
baseToModif BaseHP = Nothing
baseToModif BaseAttack = Just Attack'
baseToModif BaseDefence = Just Defence'
baseToModif BaseSAttack = Just SAttack'
baseToModif BaseSDefence = Just SDefence'
baseToModif BaseSpeed = Just Speed'

modifToBase :: ModifStat -> Maybe BaseStat
modifToBase Attack' = Just BaseAttack 
modifToBase Defence' = Just BaseDefence 
modifToBase SAttack' = Just BaseSAttack 
modifToBase SDefence' = Just BaseSDefence 
modifToBase Speed' = Just BaseSpeed 
modifToBase _ = Nothing

baseStatMapAsFunction :: BaseStatMap v -> (BaseStat -> v)
baseStatMapAsFunction BaseStatMap{..} basestat = case basestat of
  BaseHP -> hp
  BaseAttack -> attack
  BaseDefence -> defence
  BaseSAttack -> sAttack
  BaseSDefence -> sDefence
  BaseSpeed -> speed
